...
print("scRNA-seq analysis: 1- Quality control")
print("from starsolo (GTF v.8.11) - EM multimapping filtered matrix")
...
ribosomal_RNA = c("tmp.1.10", "tmp.1.100", "tmp.1.40", "tmp.1.100", "tmp.1.100" , "Tb927.2.1931", "Tb927.2.1942", "Tb927.2.1953" , "Tb927.2.1964", "Tb927.2.1975", "Tb927.2.1986", "Tb927.2.1997",
                   ...
                   "Tb09_rRNA_3"  , "Tb10_rRNA_1:rRNA", "Tb10_rRNA_2:rRNA", "Tb10_rRNA_2:rRNA" , "Tb927.11.rRNA_1", "Tb927.11.rRNA_2", "Tb927.11.rRNA_2")

mito_genes = c("MT-12SrRNA","MT-9SrRNA","MT-ND8","MT-ND9", "MT-MURF5","MT-ND7", "MT-CO3","MT-CYTB", "MT-ATP6","MT-MURF1","MT-CR3","MT-ND1", 
...
option_list = list(make_option(c("-d", "--inpdir"), default = NULL, help = "path to input directory"),
                   ...
                   make_option(c("-v", "--variablegenes"), type = "integer", default = NULL, help = "variable genes"),
                                    ...
                                    outdir = "output_analysis")
{
  ...
  rrna_perct_plot =  ggplot(seurat_obj@meta.data, aes(x = nCount_RNA, y = percent.rRNA)) +
    ...
    geom_vline(aes(xintercept=6000), color = "darkgreen", linetype="longdash")  +
    geom_hline(aes(yintercept=75), color = "grey", linetype="longdash") +
    geom_vline(aes(xintercept=(1000)), color = "grey", linetype="longdash") + 
    labs(title = "Relationship of ribosomal RNA percentage and UMI count per cell:", 
         x =  "UMI Count", y = "% rRNA genes") 
  ...
  message("Normalization, identify variable genes, regress out from rRNA ...")
  seurat_obj = NormalizeData(object = seurat_obj, normalization.method = "LogNormalize", scale.factor = 10000)
  seurat_obj = FindVariableFeatures(object = seurat_obj, selection.method = "vst", nfeatures = variable.genes)
  
  all.genes = rownames(seurat_obj)
  seurat_obj = ScaleData(seurat_obj, features = all.genes, vars.to.regress = "percent.rRNA")
  
  saveRDS(seurat_obj, paste0(outdir,"/" ,seurat_obj@project.name, "_qc_lognorm_regressout.RDS"))
...
seuobj_filtered = runQualityControlSeurat(input_matrix = opt$inpdir, 
                                          ...
                                          variable.genes = opt$variablegenes,
                                          outdir = opt$outdir)

#Rscript quality_control_norm.R -d /Users/paulafernandes/projects_2022/test/raw_data/filtered_em_multi_d0 -p rbp6d0 -f 1000 -i 30 -c 1000 -m 5 -r 10 -v 2000 -o /Users/paulafernandes/projects_2022/test/output_analysis
# 
# 
...
# 
# make_option(c("-i", "--countmin"),  default=NULL, help="count min cutoff"),
# 
...
# 
# make_option(c("-v", "--variablegenes"), default=NULL, help="variable genes"),
# 
# make_option(c("-o", "--outdir"), default=NULL, help="output directory"))
